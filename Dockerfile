...
# Через docker compose аргумент задаётся сервису nginx — именно его образ (стадия web)
# раздаёт бандл браузеру. У сервиса app то же значение указано только для согласованности:
# лежащая в нём копия статики никому не отдаётся.
...
COPY server ./
# Справочная копия статики: браузеру она НЕ отдаётся — Fastify статику не раздаёт,
# а порт 3000 наружу не публикуется. Нужна только чтобы убедиться, что сервер и
# приложение собраны из одного исходного дерева. Раздачей занимается образ nginx,
# и адрес API для браузера задаётся при сборке ИМЕННО ЕГО (см. docker-compose.yml).
